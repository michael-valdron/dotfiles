...
## dnf ##
alias dnf-update='if [ "$(command -v dnf)" ]; then sudo dnf update; else echo "system does not use dnf" && exit 1; fi'
alias dnf-update-nokernel='if [ "$(command -v dnf)" ]; then sudo dnf update --exclude="kernel kernel-core kernel-devel kernel-devel-matched kernel-modules kernel-modules-core kernel-modules-extra"; else echo "system does not use dnf" && exit 1; fi'
